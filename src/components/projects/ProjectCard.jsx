    ...
    <div ref={containerRef} className="relative w-full max-w-5xl mx-auto flex flex-col items-center group">
      ...
      <div className={cn("relative z-10 w-full grid grid-cols-1 md:grid-cols-2 gap-12 items-center", 
        ...
        <div className={cn("flex flex-col p-6", !isEven && "md:col-start-2")}>
          ...
          <button className={cn(
            "proj-card-element self-start px-8 py-4 rounded-full border text-[10px] font-bold uppercase tracking-[0.3em] transition-all duration-500 bg-transparent hover:text-black hover:bg-white border-white/20"
          )}>
            View Case Study
          </button>
        </div>
          ...
          >
            ...
            <div className="relative w-full h-full rounded-[2.3rem] overflow-hidden bg-black/40 group">
              ...
              <div className="proj-content-layer absolute bottom-8 left-8 z-20 flex flex-col gap-4" style={{ transformStyle: 'preserve-3d', transform: 'translateZ(90px)' }}>
                {/* Graphical Download or Coming Soon Box */}
                {project.downloadUrl ? (
                  <a 
                    href={project.downloadUrl}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex items-center gap-3 px-5 py-2.5 rounded-xl bg-white/10 border border-white/20 backdrop-blur-xl group/btn transition-all duration-500 hover:bg-white/20 hover:scale-105 active:scale-95 shadow-[0_10px_30px_rgba(0,0,0,0.3)]"
                  >
                    ...
                    <div className="flex flex-col items-start leading-none text-white">
                      <span className="text-[7px] uppercase tracking-wider opacity-60">Get it on the</span>
                      <span className="text-xs font-bold tracking-tight">App Store</span>
                    </div>
                  </a>
                ) : (
                  <div className="px-5 py-2.5 rounded-xl bg-white/5 border border-white/10 backdrop-blur-md opacity-60">
